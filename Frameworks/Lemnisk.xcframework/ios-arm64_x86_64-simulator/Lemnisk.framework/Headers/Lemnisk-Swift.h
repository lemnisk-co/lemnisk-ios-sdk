...
@class NSString;
SWIFT_CLASS("_TtC7Lemnisk17DeliveredMIDStore")
@interface DeliveredMIDStore : NSObject
+ (void)addMID:(NSString * _Nonnull)mid;
+ (void)sendReconciliationEvent;
- (nonnull instancetype)init OBJC_DESIGNATED_INITIALIZER;
@end

@class WKWebView;
@protocol WKURLSchemeTask;
SWIFT_CLASS("_TtC7Lemnisk23InAppAssetSchemeHandler") SWIFT_AVAILABILITY(ios,introduced=11.0)
@interface InAppAssetSchemeHandler : NSObject <WKURLSchemeHandler>
...
SWIFT_CLASS("_TtC7Lemnisk13InAppJsBridge")
@interface InAppJsBridge : NSObject <WKScriptMessageHandler>
...
@class WKNavigation;
@class NSBundle;
SWIFT_CLASS("_TtC7Lemnisk24InAppPopupViewController")
@interface InAppPopupViewController : UIViewController <WKNavigationDelegate>
- (nullable instancetype)initWithCoder:(NSCoder * _Nonnull)coder SWIFT_UNAVAILABLE;
- (void)loadView;
- (void)viewDidLoad;
- (void)viewDidDisappear:(BOOL)animated;
/// WebView-boundary hardening (mirrors Android ab6d2cd36
/// WebViewClient.shouldOverrideUrlLoading → openExternalUrl).
/// The popup is a chromeless full-screen WebView. Without this gate, an
/// <code><a href></code> / <code>window.location</code> inside campaign HTML would navigate it in
/// place and the <code>LemniskInApp</code> JS bridge would stay attached to the new
/// (third-party / redirector) document — which could then call <code>form(...)</code>
/// (writing attacker-chosen customerProperties into the user’s CDP profile)
/// or <code>cta(...)</code> (firing arbitrary intents / tel:, sms:, etc.). So the ONLY
/// document this WebView is ever allowed to host is the initial marketer
/// HTML; every other navigation is cancelled and routed out to the system
/// browser instead.
...
/// Whether the SDK has been configured (via auto-init or an explicit
/// configure()). Lets the RN/Flutter bridge proactively skip its JS/Dart
/// init when native auto-init already ran. Belt-and-suspenders — the
/// identity guard in configureBasic already makes a redundant configure()
/// a safe no-op.
@property (nonatomic, readonly) BOOL isConfigured;
SWIFT_CLASS_PROPERTY(@property (nonatomic, class, readonly, copy) NSString * _Nullable currentContext;)
+ (NSString * _Nullable)currentContext SWIFT_WARN_UNUSED_RESULT;
...
/// Set the current SDK context (e.g. “checkout_flow”, “kyc_step1”).
/// Attached as the <code>appContext</code> field on outbound track/screen events so
/// the backend can index it as a trigger-condition selector in the
/// marketer UI. Also forwarded into the in-app engine for screen_load
/// <code>contexts</code> membership checks. Pass nil or call <code>clearContext()</code> to
/// clear.
- (void)setContext:(NSString * _Nullable)contextValue;
- (void)clearContext;
/// Silence the in-app channel; popups stay suppressed until <code>resumeInApp()</code>.
- (void)pauseInApp;
/// Re-enable the in-app channel after a <code>pauseInApp()</code>.
- (void)resumeInApp;
...
/// warning:
/// Bootstrap setup (swizzling, certificate pinning) runs asynchronously after
/// the remote config fetch. Prefer <code>configure(writeKey:serverUrl:completion:)</code> and
/// defer any SDK calls to inside the completion block.
- (void)configureWithWriteKey:(NSString * _Nonnull)writeKey serverUrl:(NSString * _Nonnull)serverUrl;
/// Initializes the SDK and fetches remote configuration from <code>{serverUrl}/sdkinitialize/{writeKey}</code>.
/// The SDK initializes immediately with defaults; remote config is applied asynchronously.
/// If the remote fetch fails the SDK continues with default settings.
/// The completion is always called on the main thread.
- (void)configureWithWriteKey:(NSString * _Nonnull)writeKey serverUrl:(NSString * _Nonnull)serverUrl completion:(void (^ _Nonnull)(BOOL))completion;
...
/// Single-dictionary runtime configure (ObjC <code>configureWithConfig:</code>), used by
/// the React Native / Flutter wrappers. Reads friendly keys and maps them to
/// the runtime configure path. Honors the auto-init ownership guard: if
/// auto-init owns initialization (armed via Info.plist), this is dropped.
/// Rule: an omitted key does NOT override the current value.
/// Keys: writeKey, serverUrl (required); consentUrl, notificationCenterUrl,
/// enablePush, enablePullNotifications, enableAnalytics, enableInApp,
/// enableDebugMode. (appGroupId is NOT here — it’s iOS-specific and comes
/// from Info.plist <code>LemniskAppGroupId</code> or a setAppGroupID call.)
- (void)configureWithConfig:(NSDictionary<NSString *, id> * _Nonnull)config;
- (void)registerLemniskNotificationTasks;
/// Zero-touch bootstrap from Info.plist. Invoked once at app launch by the
/// <code>LemniskAutoInit</code> <code>+load</code> observer (UIApplicationDidFinishLaunching).
/// Contract — additive and backward-safe:
/// <ul>
///   <li>
///     No <code>LemniskWriteKey</code> in the host’s Info.plist  → silent no-op. An app
///     that upgrades the SDK without adding plist keys is unaffected; its
///     manual <code>configure(...)</code> keeps working exactly as before.
///   </li>
///   <li>
///     Host already called <code>configure(...)</code> (their didFinishLaunching runs
///     before this notification) → <code>hasConfiguredOnce</code> is set, so we stand
///     down and defer entirely to the host.
///   </li>
///   <li>
///     Otherwise → behaves identically to a manual
///     <code>configure(writeKey:serverUrl:)</code>, plus registers background tasks.
///   </li>
/// </ul>
+ (void)autoBootstrapFromInfoPlistWithLaunchOptions:(NSDictionary * _Nullable)launchOptions;
- (void)setCertificatePinningEnabled:(BOOL)enabled;
- (void)setupNotificationDelegates;
/// Returns whether remote configuration has been fetched
- (BOOL)isRemoteConfigurationFetched SWIFT_WARN_UNUSED_RESULT;
/// Returns whether analytics is enabled (from remote config or default true)
- (BOOL)isAnalyticsEnabled SWIFT_WARN_UNUSED_RESULT;
/// Returns whether crash reporting is enabled (from remote config or default true)
- (BOOL)isCrashReportingEnabled SWIFT_WARN_UNUSED_RESULT;
/// Returns whether in-app messaging is enabled (from remote config or default true)
- (BOOL)isInAppMessagingEnabled SWIFT_WARN_UNUSED_RESULT;
/// Returns whether session tracking is enabled (from remote config or default true)
- (BOOL)isSessionTrackingEnabled SWIFT_WARN_UNUSED_RESULT;
/// Returns whether push notifications are enabled (from remote config)
- (BOOL)isPushEnabled SWIFT_WARN_UNUSED_RESULT;
/// Returns whether geofencing is enabled (from remote config)
- (BOOL)isGeoFenceEnabled SWIFT_WARN_UNUSED_RESULT;
- (nonnull instancetype)init OBJC_DESIGNATED_INITIALIZER;
@end
...
/// Outcome indicator (kept public for API compatibility): true once any
/// swizzle domain has actually attached. No longer a gate — callers just
/// invoke swizzleDelegateMethods(), which is internally locked and
...
- (void)checkAndHandlePermissionChange;
